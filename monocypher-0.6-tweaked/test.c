...
{
    vector v = vec_new(256);
    next_number(s);
    for (unsigned int i = 0; stream_peek(s) != ':' && i < 256; i++) {
                        ...
                        const char * filename, size_t nb_vectors)
{
    ...
    for (unsigned int max_tests = 4; stream_peek(&stream) != EOF && max_tests > 0; max_tests--) {
...
static int test_x25519()
{
    ...
    // FRAMA-C: do not iterate 1000 times
    FOR (i, 1, 1) { iterate_x25519(k, u); }
...
{
    int status = 0;
#if (defined FC_TEST_1) || (defined FC_TEST_ALL)
    status |= generic_test(equal, "vectors_test_equal"  , 2);
#endif
#if (defined FC_TEST_2) || (defined FC_TEST_ALL)
    status |= generic_test(diff , "vectors_test_diff"   , 2);
#endif
#if (defined FC_TEST_3) || (defined FC_TEST_ALL)
    status |= test(chacha20     , "vectors_chacha20"    , 2);
#endif
#if (defined FC_TEST_4) || (defined FC_TEST_ALL)
    status |= test(hchacha20    , "vectors_h_chacha20"  , 2);
#endif
#if (defined FC_TEST_5) || (defined FC_TEST_ALL)
    status |= test(xchacha20    , "vectors_x_chacha20"  , 2);
#endif
#if (defined FC_TEST_6) || (defined FC_TEST_ALL)
    status |= test(blake2b      , "vectors_blake2b"     , 2);
#endif
#if (defined FC_TEST_7) || (defined FC_TEST_ALL)
    status |= test(blake2b_easy , "vectors_blake2b_easy", 1);
#endif
#if (defined FC_TEST_8) || (defined FC_TEST_ALL)
    status |= test(poly1305     , "vectors_poly1305"    , 2);
#endif
#if (defined FC_TEST_9) || (defined FC_TEST_ALL)
    status |= test(argon2i      , "vectors_argon2i"     , 6);
#endif
#if (defined FC_TEST_10) || (defined FC_TEST_ALL)
    status |= test(x25519       , "vectors_x25519"      , 2);
#endif
#if (defined FC_TEST_11) || (defined FC_TEST_ALL)
    status |= test(key_exchange , "vectors_key_exchange", 2);
#endif
#if (defined FC_TEST_12) || (defined FC_TEST_ALL)
    status |= test(sha512       , "vectors_sha512"      , 1);
#endif
#if (defined FC_TEST_13) || (defined FC_TEST_ALL)
    status |= test(ed25519_key  , "vectors_ed25519_key" , 1);
#endif
#if (defined FC_TEST_14) || (defined FC_TEST_ALL)
    status |= test(ed25519_sign1, "vectors_ed25519_sign", 3);
#endif
#if (defined FC_TEST_15) || (defined FC_TEST_ALL)
    status |= test(ed25519_sign2, "vectors_ed25519_sign", 3);
#endif
#if (defined FC_TEST_16) || (defined FC_TEST_ALL)
    status |= test_x25519();
#endif
#if (defined FC_TEST_17) || (defined FC_TEST_ALL)
    status |= test_aead();
#endif
