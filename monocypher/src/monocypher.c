...
static void blake2b_update(crypto_blake2b_ctx *ctx,
                           const u8 *message, size_t message_size)
{
    // -1 alarm with, but much slower: @ loop unroll 128;
    FOR (i, 0, message_size) {
...
void crypto_blake2b_final(crypto_blake2b_ctx *ctx, u8 *hash)
{
    ...
    // no alarms lost: @ loop unroll 128;
    ...
    // no alarms lost: @ loop unroll 8;
    FOR (i, 0, nb_words) {
        store64_le(hash + i*8, ctx->hash[i]);
    }
    // no alarms lost: @ loop unroll 64;
...
static void fe_pow22523(fe out, const fe z)
{
    ...
    fe_sq(t1, t0);
    //@ loop unroll 5;
    FOR (i, 1,   5) fe_sq(t1, t1);  fe_mul(t0, t1, t0);
    fe_sq(t1, t0);
    //@ loop unroll 10;
    FOR (i, 1,  10) fe_sq(t1, t1);  fe_mul(t1, t1, t0);
    fe_sq(t2, t1);
    //@ loop unroll 20;
    FOR (i, 1,  20) fe_sq(t2, t2);  fe_mul(t1, t2, t1);
    fe_sq(t1, t1);
    //@ loop unroll 10;
    FOR (i, 1,  10) fe_sq(t1, t1);  fe_mul(t0, t1, t0);
    fe_sq(t1, t0);
    //@ loop unroll 50;
    FOR (i, 1,  50) fe_sq(t1, t1);  fe_mul(t1, t1, t0);
    fe_sq(t2, t1);
    //@ loop unroll 100;
    FOR (i, 1, 100) fe_sq(t2, t2);  fe_mul(t1, t2, t1);
    fe_sq(t1, t1);
    //@ loop unroll 50;
    FOR (i, 1,  50) fe_sq(t1, t1);  fe_mul(t0, t1, t0);
    fe_sq(t0, t0);
    //@ loop unroll 2;
    FOR (i, 1,   2) fe_sq(t0, t0);  fe_mul(out, t0, z);
    WIPE_BUFFER(t0);
    WIPE_BUFFER(t1);
    WIPE_BUFFER(t2);
}
                                         ...
                                         u8 p[32], u8 b[32])
{
    ...
    int i = 253;
    // no alarms lost: @ loop unroll 253;
    while (i >= 0         &&
           p_adds[i] == 0 &&
           b_adds[i] == 0) {
        i--;
