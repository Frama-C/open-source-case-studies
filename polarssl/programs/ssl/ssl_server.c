...
// Commenting out unsupported functions with varargs
#ifdef __FRAMAC__
# define printf(...)
# define snprintf(...)
# define sprintf(buf, ...) my_sprintf(buf)
//@ ghost volatile int _state;
/*@
  requires \valid(str+(0 .. 5)); // we define an arbitrary length
  assigns \result, _state, *(str + (0 .. 5)) \from _state;
  ensures \initialized(str + (0 .. 5));
 */
...
#define HTTP_RESPONSE					 \
    ...
    "<html>\r\n" \
    "<body>\r\n" \
    "<h2>This is a test server</h2>\r\n" \
    "<h2>Current server time: %s</h2>\r\n" \
...
/*@
  ...
  ensures \initialized((ssl->session)->master + (0 .. 47));
...
int main( int argc, char *argv[] )
{
    ...
    /* To print time information in the server */
    time_t t;
    char *stime;
...
reset:
    ...
    time(&t);
    stime = ctime(&t);
    len = sprintf( (char *) buf, HTTP_RESPONSE, stime,
